...
- docs/guide/: 利用者向けの詳細情報（CLIコマンド一覧など）
- docs/api/: モジュールごとのAPIリファレンス（mkdocstringsで自動生成）
- docs/development/: 開発者向けの情報（セットアップ・モジュール追加手順・リリース手順など）

README.mdとdocs側（とくに`docs/index.md`）で概要・特徴・インストール手順・extras一覧が部分的に重複する場合がある。README.mdはGitHubトップとして、docs側は公開ドキュメントサイトの入口としてそれぞれ自己完結する必要があるため、この重複は許容する。
